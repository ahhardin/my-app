...
const handleEvent = ({ name, data}, state) => {
  if (name === 'addMeal') {
    const newMeal = {}
    newMeal.id=data.id
    newMeal.title=data.title
    newMeal.extendedIngredients=data.extendedIngredients
    newMeal.instructions=data.instructions
    state.meals = state.meals.concat([newMeal]);
  }
  else {
    // If we don't know what kind of event this is, alert the developer!
    throw new Error(`Unrecognized event: ${name}`);
